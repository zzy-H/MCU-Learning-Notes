...
#define configCHECK_FOR_STACK_OVERFLOW 						2	// 大于 0 时启用堆栈溢出检测功能，如果使用此功能用户必须提供一个栈溢出钩子函数如果使用的话此值可以为 1 或者 2，因为有两种栈溢出检测方法
...
#define configCPU_CLOCK_HZ			( ( unsigned long ) SystemCoreClock )	//写入实际的CPU 内核时钟频率，也就是 CPU 指令执行频率
...
#define configTOTAL_HEAP_SIZE		( ( size_t ) ( 16 * 1024 ) )//系统所有总的堆大小
