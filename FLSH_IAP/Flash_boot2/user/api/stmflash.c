...
// WriteAddr 地址必须是2K的整数倍
//NumToWrite 要擦除的内存大小，必须是2K的整数倍
//该函数的使用前提是扇区已经被擦除过
uint8_t STMFLASH_WriteEasy(u32 WriteAddr, u16 *pBuffer, u16 NumToWrite)	
{
	u16 * ppp = pBuffer;
	uint16_t i=0;
	uint16_t len = NumToWrite/STM_SECTOR_SIZE;
	if(WriteAddr<STM32_FLASH_BASE||(WriteAddr>=(STM32_FLASH_BASE+1024*STM32_FLASH_SIZE)))
		return -1;//非法地址
	...
	FLASH_Unlock();//解锁
	for(i=0; i<len; i++) {
		FLASH_ErasePage(WriteAddr+i*STM_SECTOR_SIZE);//擦除这个扇区
		STMFLASH_Write_NoCheck(WriteAddr+i*STM_SECTOR_SIZE, ppp, STM_SECTOR_SIZE);
		ppp+=STM_SECTOR_SIZE;
	}
	FLASH_Lock();//上锁	
	return 0;
