...
- [一、UART\USART](#一uartusart)
  - [1、UART是什么](#1uart是什么)
  - [2、硬件连接](#2硬件连接)
  - [3、概念分层](#3概念分层)
  - [4、UART帧格式](#4uart帧格式)
  - [5、波特率](#5波特率)
  - [6、MCU处理串口数据的三种方式](#6mcu处理串口数据的三种方式)
  - [7、UART 配置步骤（以 STM32 为例）](#7uart-配置步骤以-stm32-为例)
- [二、IIC(I2C)](#二iici2c)
  - [1、I2C 是什么？](#1i2c-是什么)
  - [2、硬件连接](#2硬件连接-1)
  - [3、I2C 的速率模式](#3i2c-的速率模式)
  - [4、I2C 协议与时序](#4i2c-协议与时序)
  - [5、I2C 的读写流程](#5i2c-的读写流程)
  - [6、I2C 配置示例（以 STM32 标准库库为例）](#6i2c-配置示例以-stm32-标准库库为例)
  - [7、实际应用](#7实际应用)
    - [7.1、触摸引脚初始化（短时复位）](#71触摸引脚初始化短时复位)
    - [7.2、写寄存器](#72写寄存器)
    - [7.3、读寄存器](#73读寄存器)
- [三、SPI](#三spi)
  - [1、SPI是什么](#1spi是什么)
  - [2、硬件连接](#2硬件连接-2)
  - [3、SPI 协议与时序](#3spi-协议与时序)
  - [4、SPI 通信流程](#4spi-通信流程)
  - [5、SPI 配置示例（以 STM32 标准库库为例）](#5spi-配置示例以-stm32-标准库库为例)
  - [6、完整的 SPI 通信流程（以 W25Q64 读取 ID 为例）](#6完整的-spi-通信流程以-w25q64-读取-id-为例)
- [四、CAN和FDCAN](#四can和fdcan)
  - [1、CAN是什么](#1can是什么)
  - [2、CAN物理层](#2can物理层)
  - [3、CAN 硬件连接（收发器的作用）](#3can-硬件连接收发器的作用)
  - [4、CAN 协议特点](#4can-协议特点)
  - [5、CAN 报文结构（帧格式）](#5can-报文结构帧格式)
  - [6、CAN 位时序与同步](#6can-位时序与同步)
  - [7、FDCAN\CAN 配置逐行解析](#7fdcancan-配置逐行解析)
  - [8、FDCAN 与 CAN-FD 的关系（概念澄清）](#8fdcan-与-can-fd-的关系概念澄清)
  - [9、CAN 收发器详解](#9can-收发器详解)
  - [10、调试经验与踩坑](#10调试经验与踩坑)
- [五、扩展TCP/IP](#五扩展tcpip)
  - [1、TCP/IP是什么？](#1tcpip是什么)
  - [2、分层模型](#2分层模型)
  - [3、TCP 与 UDP 对比](#3tcp-与-udp-对比)
  - [4、嵌入式中的 TCP/IP 实现](#4嵌入式中的-tcpip-实现)
  - [5、详细解析TCP核心机制](#5详细解析tcp核心机制)
    - [5.1 三次握手（建立连接）](#51-三次握手建立连接)
    - [5.2 四次挥手](#52四次挥手)
  - [6、MQTT 协议详解](#6mqtt-协议详解)
    - [6.1 层次关系](#61-层次关系)
    - [6.2 核心概念（发布/订阅）](#62-核心概念发布订阅)
    - [6.3 MQTT 报文类型](#63-mqtt-报文类型)
    - [6.4 MQTT 四步（cloud.c 真实代码）](#64-mqtt-四步cloudc-真实代码)
- [补充](#补充)
  - [1、为什么RS-485总线首尾要接 120Ω 终端电阻](#1为什么rs-485总线首尾要接-120ω-终端电阻)
  - [2、GT911 的 I2C 地址选择机制](#2gt911-的-i2c-地址选择机制)
  - [3、GT911 坐标解析细节](#3gt911-坐标解析细节)
